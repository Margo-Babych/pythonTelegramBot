...
choice = {}
global_users_dict = {}
CATEGORIES = ['Food', 'Household good', 'Transport', 'Clothes', 'Car', 'Sport', 'Lessons', 'Taxes']
SHOW_MODE = ['Day','Week', 'Month']
...
def command_start(m):
    ...
    help_text = "Welcome! Let's count the costs\nThe following commands are available: \n\n"
...
def process_show_spending(message):
    try:
        ...
        elif DayWeekMonth == 'Week':
            today = datetime.now().today()
            startOfWeek = today - timedelta(days=today.weekday())
            endOfWeek = startOfWeek + timedelta(days=6)
            queryResult = [value for index, value in enumerate(history) if
                           startOfWeek.strftime(dateFormat) <= value.split(',')[0] <= endOfWeek.strftime(
                               dateFormat)]  # query all within the current week
